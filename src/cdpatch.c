...
static uint8_t* read_cooked_sector(
    ...
    if(!data) {
        ...
        if(bin->type == BINTYPE_2048) {
            ...
            // Verify the EDC
            //
            if(!opt->skipfscheck && edc_verify(data)) {
                printf("Error: CD sector %lu is corrupt%s\n",
                    (unsigned long)sector, fsoverride
                );
                goto error;
            }
        }
    ...
    if(bin->type == BINTYPE_2352) {
        ...
        case 1:
            ...
            break;
        case 2:
            if(!opt->skipfscheck && (data[0x12] & 0x20)) {
                printf("Error: Attempted to read Form 2 sector %lu%s\n",
                    (unsigned long)sector, fsoverride
                );
                goto error;
            }
        ...
        default:
            if(!opt->skipfscheck) {
                printf("Error: Invalid mode 0x%02X at sector %lu%s\n",
                    (int)data[0xF], (unsigned long)sector, fsoverride
                );
                goto error;
            }
...
static int extract_file(
    ...
    if(data && should_use_riff_format(data)) {
        ...
        for(; sectors; sector++, sectors--) {
            ...
            if(!data) { goto error; }
            if(!opt->skipfscheck && edc_verify(data)) {
                printf("Error: %s: CD sector %lu is corrupt%s\n",
                    filename, (unsigned long)sector, fsoverride
                );
                goto error;
            }
...
static void dofile(
    ...
    if(opt->extract) {
        ...
        if(filenode) {
            if(extract_file(
                bin, filenode->sector, filenode->size, filename, opt
            )) { goto error; }
        } else {
            if(extract_file(bin, 0, 32768, filename, opt)) { goto error; }
        }
        (*numsuccesses)++;
